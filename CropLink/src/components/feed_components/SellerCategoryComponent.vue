<template>
    <template v-if="!isLoading">
        <div class="flex flex-wrap w-full">
            <span v-for="(variety) in produce" class="relative inline-block w-24" @click="()=>selectedCategory = variety.id">
                ...
                <img :src="variety.imageUrl" class="w-24 h-24"/>
            </span>
        </div>
        <div class="flex flex-row gap-2 h-16 p-2">
            <div class="w-1/6"></div>
            <Listbox
            :items="produce"
        ...
        <span class="flex flex-row gap-2 relative w-full">
            <div class="w-1/6 py-4 px-2 sticky top-24 h-64">
                ...
                <span class="divide-y">
                    <div class="py-4">
                        <p class="text-sm mb-2">Yield Tonnage</p>
                        <div class="flex flex-row gap-2 items-center">
                            <input type="number" class="text-xs border rounded-md p-2 w-1/2" v-model="filters.yieldTonnage.min" />
                            <p>-</p>
                            <input type="number" class="text-xs border rounded-md p-2 w-1/2" v-model="filters.yieldTonnage.max"/>
                        </div>
                    </div>
                    <div class="py-4">
            ...
            <div class="grid sm:grid-cols-2 md:grid-cols-3 w-5/6 gap-2" v-if="!isLoadingAds && filteredAds.length > 0" id="sellerAds">
                <SellerAdThumbnailCard v-for="(ad,index) in filteredAds" :ad="ad" :key="index" :showButtons="false"/>
            </div>
            <div v-if="selectedCategory && isLoadingAds" class="w-5/6">
                <div class="h-96 w-full flex items-center justify-center">
                    <LoadingSpinner :isLoading="isLoadingAds"/>
                </div>
            </div>
            <div v-if="selectedCategory && !isLoadingAds && filteredAds.length == 0" class="w-5/6">
                <div class="h-96 w-full flex items-center justify-center">
...
import { ref as vueRef, onMounted, type PropType, watch, computed, reactive } from 'vue';
...
const filteredAds = computed(() => {
    ...
    const filtered =  ads.value.docs.filter((ad) => {
        const yieldTonnage = ad.yieldTonnage;
        const pricePerTon = ad.price;
        return yieldTonnage >= filters.yieldTonnage.min && yieldTonnage <= filters.yieldTonnage.max
        && pricePerTon >= filters.pricePerTon.min && pricePerTon <= filters.pricePerTon.max
    })
